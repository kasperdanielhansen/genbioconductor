...
(the `table(table())` construction may seems weird at first, but its a great way to get a quick tabular summary of occurences with the same name).  This shows that almost all genes have a single transcript, which reflects how conservative RefSeq is.
...
Now, the overlap is non-empty, but is it interesting or "significant".  Answering this question thorughly requires thinking deeply about background distributions etc.  We will avoid a careful statistical approach to this question, and will instead do a few backk-of-the-envelope calculations to get a feel for it.
...
(result is in megabases; the humnan genome is 3000 megabase).  Just this fact alone should convince you that the overlap is highly unlikely happen purely by chance. Let us calculate the overlap in megabases:
```{r size}
...
Here we basically says that each base can be assigned to peaks or promoters independently, which is definately false.  So there are many, many reasons why this calculation is not the "right" one.  Nevertheless, it appears that we have some enrichment, as expected.
