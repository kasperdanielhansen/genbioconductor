...
The `r Biocpkg("GRanges")` vignette from the [GenomicRanges webpage](http://bioconductor.org/packages/GenomicRanges).
...
Lets clean up `gr2` because we use it below
```{r cleanup}
genome(gr2) <- "hg19"
```

# GRanges, seqinfo and genomeInfoDb
...
This is one reason the `GenomeInfoDb` package was created.  It contains information about the "standard" chromosomes from widely used organisms.

FIXME: add section on GenomeInfoDb
...
Notice how the first element of `gr` does not overlap with the first element of `gr2`.  This is because by default a strand of `*` does not overlap a strand of `+`.  This might be confusing.  One way of fixing this is by using the `ignore.strand` argument to `findOverlaps`:

```{r findOverlaps_ignore}
findOverlaps(gr, gr2, ignore.strand = TRUE)
```

Several other functions in `GenomicRanges` have an `ignore.strand` argument for this reason.
