...
The `r Biocpakg("Biostrings")` package contains classes and functions for representing biological strings such as DNA, RNA and amino acids.  In addition the package has functionality for pattern matching (short read alignment) as well as a pairwise alignment function implementing Smith-Waterman local alignments and Needleman-Wunsch global alignments used in sequencing alignment.  There are also functions for reading and writing output such as FASTA files.
...
Note that the alphabet of a `DNAString` is an extended alphabet: `-` (for insertion) and `N` are allowed.  In fact, IUPAC codes are allowed (these codes represets different characters, for example the code "M" represents either and "A" or a "C").  A list of IUPAC codes can be obtained by

```{r IUPAC}
IUPAC_CODE_MAP
```

Indexing into a `DNAString` retrives a subsequence (similar to the standard R function `substr`), whereas indexing into a `DNAStringSet` gives you a subset of sequences.  Note that `[[` allows you to get a single element of a `DNAStringSet` as a `DNAString`.  This is very similar to `[` and `[[` for lists.
...
The `available.genomes()` function lists which genomes are currently available pre-packages from Bioconductor.  Note that there are several so-called "masked" genomes, where some parts of the genome are masked.  We will avoid this subject for now.
