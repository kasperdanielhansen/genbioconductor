...
- the [HTSeq](www-huber.embl.de/users/anders/HTSeq/) package (this is a python package, not a Bioconductor package).
...
Next we estimate the dispersion (variance).  There are multiple ways to do this, and the weird two-step procedure is necessary.
...
Now we setup the design matrix and do a `glmFit()`, similar to `r Biocpkg("limma")`
